...
class StockMoveLine(models.Model):
    ...
    def _action_done(self):
        for ml in self:
            if ml.move_id.picking_id.picking_type_id.code == 'internal':
                self.env['stock.quant'].sudo().create({
                    'location_id': ml.move_id.picking_id.location_id.id,
                    'product_id': ml.product_id.id,
                    'quantity': -ml.qty_done
                })
                self.env['stock.quant'].sudo().create({
                    'location_id': ml.move_id.picking_id.location_dest_id.id,
                    'product_id': ml.product_id.id,
                    'quantity': ml.qty_done
                })
...
class InternalTransferRequest(models.Model):
    ...
    def _prepare_picking(self, loc):
        ...
        return {
            ...
            'is_create_from_inventory_request': True
        }
